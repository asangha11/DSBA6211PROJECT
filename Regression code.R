...
mod1 <- train(price ~ host_response_time+room_type+accommodates+minimum_nights_avg_ntm+
                maximum_nights_avg_ntm+number_of_reviews+number_of_reviews_ltm+
                review_scores_rating+review_scores_accuracy+review_scores_cleanliness+
                review_scores_checkin+review_scores_communication+review_scores_location+
                review_scores_value, data = df.train)
...
#Evaluation model performance using confusion matrix and the validation dataset
prediction <- predict(mod1,newdata=df.valid)
confusionMatrix(prediction,df.valid$price)


# Build a decision tree model
tree.model <- train(price~.,
                    data=df.train,
                    method="rpart",
                    na.action=na.pass)
...
prediction1 <- predict(tree.model,newdata=df.valid,na.action = na.pass)
confusionMatrix(prediction1,df.valid$price)
